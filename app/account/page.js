      ...
      <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
        <div className="bg-white p-8 md:p-10 rounded-[2.5rem] md:rounded-[3rem] shadow-sm max-w-md w-full border border-gray-100">
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            ...
            <button
              disabled={loading}
              className={`w-full py-4 rounded-xl font-black uppercase tracking-widest text-[10px] md:text-xs text-white shadow-xl transition-all ${
      ...
      <div className="max-w-6xl mx-auto px-4 py-8 md:py-10">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6 mb-10 md:mb-12">
          ...
          <button 
            ...
            className="text-[10px] md:text-xs font-black uppercase tracking-widest text-red-500 bg-red-50 px-6 py-3 rounded-xl hover:bg-red-500 hover:text-white transition w-fit"
          >
            Sign Out
          </button>
        </div>
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            {orders.length === 0 ? (
              <div className="bg-white p-10 md:p-16 rounded-[2rem] md:rounded-[2.5rem] text-center border-2 border-dashed border-gray-100">
                <p className="text-gray-400 font-bold uppercase tracking-widest text-[10px] md:text-xs">You haven't ordered any lamps yet.</p>
                <Link href="/shop" className="bg-black text-white px-8 py-3.5 rounded-xl font-black uppercase tracking-widest text-[10px] mt-6 inline-block hover:bg-gray-800 transition">Shop New Lamps</Link>
              </div>
            ) : (
              orders.map((order) => (
                    ...
                    {order.items.map((item, i) => (
                      <div key={i} className="flex gap-4 md:gap-5 items-center">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-[10px] md:text-xs font-bold text-gray-400 uppercase tracking-widest">{item.quantity} x ৳{item.price.toLocaleString()}</p>
                        </div>
                      </div>
                    ))}
                  </div>
                  ...
                  {order.status === "Pending" && (
                    <div className="px-5 md:px-6 pb-5 md:pb-6 pt-2">
                      <button 
                        ...
                        className="w-full py-3 bg-red-50 text-red-500 rounded-xl font-black uppercase tracking-widest text-[10px] hover:bg-red-500 hover:text-white transition flex items-center justify-center gap-2"
                      >
                        <HiXCircle />
                        Cancel Order
                      </button>
                ...
                {wishlist.slice(0, 3).map((p) => (
                  <Link key={p._id} href={`/product/${p.slug}`} className="flex gap-4 items-center group">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mt-0.5">৳{p.price.toLocaleString()}</p>
                    </div>
                  </Link>
                ))}
                {wishlist.length > 3 && (
                  <Link href="/wishlist" className="text-[10px] font-black uppercase tracking-widest text-blue-600 hover:text-blue-700 transition block text-center pt-2">
                    View all {wishlist.length} lights
                  </Link>
                )}
                {wishlist.length === 0 && (
                  <p className="text-[10px] text-gray-400 font-bold uppercase tracking-widest italic text-center py-4">Your saved list is empty</p>
                )}
              </div>
            </div>
