            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                {stats.map((stat, i) => (
                    <div key={i} className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-gray-100 relative overflow-hidden group hover:shadow-xl transition-all duration-300">
                        ...
                        <p className="text-sm font-black text-gray-600 uppercase tracking-widest mb-1">{stat.label}</p>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
                    ...
                    <div className="overflow-x-auto">
                        <table className="w-full text-left">
                            <thead className="bg-gray-50/50">
                                <tr>
                                    <th className="px-8 py-4 text-[12px] font-black text-gray-600 uppercase tracking-widest">ID</th>
                                    <th className="px-8 py-4 text-[12px] font-black text-gray-600 uppercase tracking-widest">Customer</th>
                                    <th className="px-8 py-4 text-[12px] font-black text-gray-600 uppercase tracking-widest">Amount</th>
                                    <th className="px-8 py-4 text-[12px] font-black text-gray-600 uppercase tracking-widest">Status</th>
                                </tr>
                            </thead>
                                    ...
                                    data.recentOrders.map((order) => (
                                        <tr key={order._id} className="hover:bg-gray-50/30 transition">
                                            <td className="px-8 py-5">
                                                ...
                                                <p className="text-[12px] font-bold text-gray-600 uppercase tracking-tighter">
                                                    {new Date(order.createdAt).toLocaleDateString()}
                                                </p>
                                            </td>
                                            <td className="px-8 py-5">
                                                <p className="text-sm font-bold text-gray-900">{order.customer.name}</p>
                                                <p className="text-[12px] text-gray-600 font-medium">{order.customer.phone}</p>
                                            </td>
                                            <td className="px-8 py-5">
                ...
                <div className="space-y-8">
                    {/*<div className="bg-gray-200 text-gray-800 p-8 rounded-[3rem] shadow-2xl relative overflow-hidden">
                        ...
                        <p className="text-gray-600 text-sm font-bold mb-6 relative z-10 leading-relaxed">
                    ...
                    <div className="bg-white p-8 rounded-[3rem] shadow-sm border border-gray-100">
                        <h3 className="text-sm font-black text-gray-600 uppercase tracking-widest mb-6">Inventory Status</h3>
